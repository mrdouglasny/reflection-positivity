...
2. **Operator-coincidence lemma** (where the Codex lattice review's covariance work lands):
   ...
   **✅ NORMALIZATION RESOLVED (2026-06-03, pphi2 `bb4b86d`).** The earlier `a`-power
   mismatch is fixed: `asymTransferWeight` is now `exp(−(a²/2)·spatialAction)` (matching
   the Gibbs `a²·Σwick` / precision `a²Q`, GJ `aᵈ` convention), verified by the 2-slice
   two-point check and re-build (`asymTransferNormalized_gap` re-proved). So
   `asymTransferOperatorCLM` is now the genuine transfer operator of the measure, and
   this coincidence lemma can carry the proved gap.
